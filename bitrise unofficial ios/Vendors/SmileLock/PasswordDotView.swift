...
open class PasswordDotView: UIView {
  ...
  open var inputDotCount: Int = 0 {
    didSet {
      let format = Bundle(for: type(of: self))
        .localizedString(forKey: "PasswordDotViewAccessibilityValue", value: nil, table: nil)
  ...
  open override func draw(_ rect: CGRect) {
    ...
    for (index, position) in positions.enumerated() {
      if index < inputDotCount {
        let pathToFill = UIBezierPath(circleWithCenter: position,
                                      radius: (radius + borderWidth / 2), lineWidth: borderWidth)
        pathToFill.fill()
      } else {
  ...
  open override func awakeFromNib() {
    ...
    accessibilityLabel = Bundle(for: type(of: self))
      .localizedString(forKey: "PasswordDotViewAccessibilityLabel", value: nil, table: nil)
  }
